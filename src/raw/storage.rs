//! Table storage: owns the physical layout of the hash table.
//!
//! This module encapsulates everything that knows about the *bytes* of the
//! table: the [`TableLayout`] arithmetic, the [`Bucket`] pointers into the
//! data area, and [`RawTableInner`], which stores the control bytes (including
//! the mirrored group at the end of the control byte array) together with the
//! `bucket_mask` / `growth_left` / `items` bookkeeping.
//!
//! Probing policy lives in [`super::probe`] and growth/rehash transactions
//! live in [`super::growth`]; this module only exposes the storage
...
mod tests {
    ...
    /// Reports the active control-group backend so that cross-backend test
    /// runs (e.g. with `RUSTFLAGS="-C target-feature=-neon"`) can be told
    /// apart in the test output.
    #[test]
    fn report_control_backend_width() {
        println!("control group backend width: {}", Group::WIDTH);
    }

    /// Locks the physical table layout: bucket counts, the size of the
    /// type-erased table header and the exact allocation sizes (including
    /// the mirrored control bytes) must not change silently.
    #[test]
    fn storage_layout_is_unchanged() {
        // The type-erased table header is exactly four words.
        assert_eq!(size_of::<RawTableInner>(), 4 * size_of::<usize>());

        // Bucket counts for `u64` elements are independent of the group
        // width of the control-byte backend.
        assert_eq!(RawTable::<u64>::new().num_buckets(), 1);
        assert_eq!(RawTable::<u64>::with_capacity(3).num_buckets(), 4);
        assert_eq!(RawTable::<u64>::with_capacity(7).num_buckets(), 8);
        assert_eq!(RawTable::<u64>::with_capacity(14).num_buckets(), 16);
        assert_eq!(RawTable::<u64>::with_capacity(100).num_buckets(), 128);

        // Allocation sizes include the `Group::WIDTH` mirrored control bytes.
        // For `u64` the control-byte alignment equals `Group::WIDTH` and the
        // data area needs no padding, so the layout is exact.
        assert_eq!(RawTable::<u64>::new().allocation_size(), 0);
        assert_eq!(
            RawTable::<u64>::with_capacity(3).allocation_size(),
            4 * size_of::<u64>() + 4 + Group::WIDTH
        );
        assert_eq!(
            RawTable::<u64>::with_capacity(14).allocation_size(),
            16 * size_of::<u64>() + 16 + Group::WIDTH
        );

        // Zero-sized types have no data area at all: the allocation is only
        // the control bytes plus the mirrored group.
        let zst_table = RawTable::<()>::with_capacity(10);
        assert_eq!(
            zst_table.allocation_size(),
            zst_table.num_buckets() + Group::WIDTH
        );
    }
}
