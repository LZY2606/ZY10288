...
pub(crate) use storage::Bucket;

use probe::ProbeCursor;
use storage::{
...
pub(crate) struct RawIterHashIndices {
    ...
    // The cursor over the sequence of groups to probe in the search.
    cursor: ProbeCursor,
...
impl RawIterHashIndices {
    ...
    unsafe fn new(table: &RawTableInner, hash: u64) -> Self {
        ...
        let cursor = table.probe_seq(hash);
        let group = unsafe { Group::load(table.ctrl(cursor.group_pos())) };
        ...
        unsafe {
            loop {
                if let Some(bit) = self.bitmask.next() {
                    let index = self.cursor.bucket_index(bit);
                    return Some(index);
                }
                ...
                let index = self.cursor.group_pos();
...
mod test_map {
    ...
    /// CHECKING THAT A PANIC IN THE HASH FUNCTION DURING GROWTH ROLLS BACK
    /// THE MIGRATION AND LEAVES THE ORIGINAL TABLE INTACT (NO LEAKS, NO
    /// DOUBLE DROPS, ALL ELEMENTS STILL REACHABLE).
    #[test]
    #[cfg(panic = "unwind")]
    fn test_growth_migration_rollback_on_panicking_hash() {
        use core::cell::Cell;
        use core::sync::atomic::{AtomicUsize, Ordering};
        use std::panic::{AssertUnwindSafe, catch_unwind};
        use stdalloc::sync::Arc;

        // Tracks the number of live instances to catch leaks and double drops.
        ...
        let mut table = RawTable::new();
        let hasher = |v: &Counted| v.0;

        // Fill the table completely so that the next `reserve` call has to
        // allocate a new table and migrate all elements into it (instead of
        // rehashing in place).
        for i in 0.. {
            table.insert(i, Counted::new(i, &live), hasher);
            if table.len() >= 100 && table.len() == table.capacity() {
                break;
            }
        }
        let items = table.len();
        assert!(items > 0);
        assert_eq!(live.load(Ordering::SeqCst), items);

        // A hasher that panics on the Nth call, i.e. in the middle of the
        // migration into the new table.
        let calls = Cell::new(3);
        let panicking_hasher = |v: &Counted| {
            assert!(calls.get() != 0, "panic in hash during growth");
            calls.set(calls.get() - 1);
            v.0
        };

        let result = catch_unwind(AssertUnwindSafe(|| table.reserve(1, panicking_hasher)));
        assert!(result.is_err());

        // The growth transaction must have rolled back: the original table
        // is unchanged and no element was leaked or dropped twice.
        assert_eq!(table.len(), items);
        assert_eq!(live.load(Ordering::SeqCst), items);
        for i in 0..items as u64 {
            assert!(table.find(i, |v| v.0 == i).is_some(), "Index: {i}");
        }

        // The table is still fully usable: growing it again (with a
        // non-panicking hasher) migrates all elements.
        table.reserve(1, hasher);
        table.insert(items as u64, Counted::new(items as u64, &live), hasher);
        for i in 0..=items as u64 {
            assert!(table.find(i, |v| v.0 == i).is_some(), "Index: {i}");
        }
    ...
    /// CHECKING THAT AN ALLOCATION FAILURE DURING GROWTH LEAVES THE
    /// ORIGINAL TABLE UNTOUCHED.
    #[test]
    fn test_growth_alloc_failure_preserves_table() {
        use core::cell::Cell;

        // An allocator that starts failing once its budget is exhausted.
        struct FailAlloc {
        ...
        // Make the next allocation (the replacement table allocated by the
        // growth transaction) fail.
        table.alloc.remaining.set(0);
        assert!(table.try_reserve(table.capacity() + 1, hasher).is_err());

        // The original table is fully intact.
        assert_eq!(table.len(), 100);
        assert_eq!(table.num_buckets(), buckets);
        for i in 0..100 {
            assert!(table.find(i, |x| *x == i).is_some(), "Index: {i}");
        }

        // With a working allocator the table grows again.
        table.alloc.remaining.set(usize::MAX);
        table.try_reserve(table.capacity() + 1, hasher).unwrap();
        for i in 0..100 {
            assert!(table.find(i, |x| *x == i).is_some(), "Index: {i}");
        }
    }
