//! Probe cursor: owns the probing policy of the hash table.
//!
//! The [`ProbeCursor`] is the single owner of the probe sequence. It only
//! produces the positions of the groups to inspect and the termination
//! condition of a probe; it never touches the table's data area or control
//! bytes itself. The lookup/insert routines below are thin combinations of a
//! [`ProbeCursor`] and the storage primitives from [`super::storage`], so the
//! unsafe invariants of probing are documented in exactly one place.
...
///
/// The cursor is the single owner of the probing policy: it produces the
/// positions of the groups to inspect ([`ProbeCursor::pos`],
/// [`ProbeCursor::move_next`]) and the termination condition of a probe
/// ([`ProbeCursor::is_terminal`]). It never loads control bytes and never
/// touches the data area of the table, which keeps the unsafe surface of the
/// table storage ([`super::storage`]) separate from the probing logic.
///
/// The probe sequence is based on triangular numbers and is guaranteed (since
/// the table size is a power of two) to visit every group of elements exactly
/// once, so a probe loop must terminate upon reaching a terminal group.
#[derive(Clone)]
...
impl ProbeCursor {
    /// Creates a cursor that starts at the group selected by `h1(hash)`.
    ///
    /// `bucket_mask` must be the `bucket_mask` of the table being probed, i.e.
    /// one less than its (power-of-two) number of buckets.
    #[inline]
    pub(super) fn new(hash: u64, bucket_mask: usize) -> Self {
        Self {
            seq: ProbeSeq {
                // This is the same as `hash as usize % num_buckets` because the
                // number of buckets is a power of two, and
    ...
    /// Returns the position of the group the cursor currently points to.
    ///
    ...
    /// Returns the bucket mask of the table this cursor probes.
    #[inline]
    ...
    ///
    /// The caller must stop the probe (see [`ProbeCursor::is_terminal`])
    /// before the sequence wraps around; the sequence visits every group
    /// exactly once.
    #[inline]
    ...
    /// Returns `true` if a probe may terminate at `group`, i.e. the group
    /// contains at least one [`Tag::EMPTY`] bucket.
    ///
    /// A probe that has not found its key must stop at the first terminal
    /// group: the key cannot be located beyond it.
    #[inline]
...
impl RawTableInner {
    ...
    #[inline]
    pub(super) unsafe fn fix_insert_index(&self, mut index: usize) -> usize {
    ...
    #[inline]
    pub(super) fn find_insert_index_in_group(
        &self,
        group: &Group,
        cursor: &ProbeCursor,
    ) -> Option<usize> {
        ...
        if likely(bit.is_some()) {
            // This is the same as `(cursor.pos() + bit) % self.num_buckets()` because the number
            // of buckets is a power of two, and `self.bucket_mask = self.num_buckets() - 1`.
            Some((cursor.pos() + bit.unwrap()) & self.bucket_mask)
        } else {
            None
        }
    }
    ...
    pub(super) unsafe fn find_or_find_insert_index_inner(
        ...
        let mut cursor = self.probe_cursor(hash);

        loop {
            // SAFETY:
            // * Caller of this function ensures that the control bytes are properly initialized.
            //
            // * `ProbeCursor::pos` cannot be greater than `self.bucket_mask = self.num_buckets() - 1`
            ...
            let group = unsafe { Group::load(self.ctrl(cursor.pos())) };

            for bit in group.match_tag(tag_hash) {
                let index = (cursor.pos() + bit) & self.bucket_mask;
    ...
    /// Returns a [`ProbeCursor`] for a probe sequence on the table.
    ///
    /// The cursor never terminates by itself, but is guaranteed to visit each
    /// bucket group exactly once. The loop using the cursor must terminate
    /// upon reaching a group for which [`ProbeCursor::is_terminal`] returns
    /// `true`.
    #[inline]
    pub(super) fn probe_cursor(&self, hash: u64) -> ProbeCursor {
        ProbeCursor::new(hash, self.bucket_mask)
    }

    #[inline]
    pub(super) fn is_in_same_group(&self, i: usize, new_i: usize, hash: u64) -> bool {
        let probe_seq_pos = self.probe_cursor(hash).pos();
        let probe_index =
